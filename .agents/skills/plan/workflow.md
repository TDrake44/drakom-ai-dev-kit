...
3. **Draft Architecture Spec:**
   - Create a scratchpad document at `.ai/plans/<feature-or-bug-name>.md`.
...
4. **Plan Storage Decision:**
   - Ask the user where the plan should be stored:
     - **`.ai/plans/`** (default) — Local-only, gitignored. Best for solo work, short-lived tasks, or exploratory spikes.
     - **`.ai/specs/`** — Git-tracked and version-controlled. Choose this when:
   ...
   - If the user selects `.ai/specs/`, move or copy the draft from `.ai/plans/` to `.ai/specs/<feature-or-bug-name>.md`.
